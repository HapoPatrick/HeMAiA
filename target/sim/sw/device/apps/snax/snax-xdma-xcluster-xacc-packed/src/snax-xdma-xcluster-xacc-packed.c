...
uint32_t start_cycle_0;
uint32_t end_cycle_0;
uint32_t elapsed_0;
...
int reshuffler_check_from_tcdm(
    ...
    int32_t total_len        // prod(tloops) * spatial_len * 8
) {
    int err = 0;
    ...
    for (int i0 = 0; i0 < tloops[0]; i0++) {
        ...
        for (int s = 0; s < spatial_len; s++) {
            ...
            for (int k = 0; k < 8; k++) {  // Compare 8 elements
                int8_t actual = tcdm_base[addr + k];
                int8_t expected = golden[result_idx + k];

                if (actual != expected) {
                    printf("❌ Mismatch at result[%d]: got %d, expected %d (addr=%d)\n",
                        result_idx + k, actual, expected, addr + k);
                    err++;
                    // return -1;
                }
            }
...
int main() {
    ...
    // if (snrt_cluster_idx() == 0) {
    //     if (snrt_is_dm_core()) {
    //         xdma_memcpy_nd((uint8_t*)tcdm0_start_addr, (uint8_t*)(tcdm0_start_addr+0x10000), spatialStride_in, spatialStride_out,
    //                 6, tstride_src, tbound_src, 6, tstride_dst, tbound_dst,
    //                 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF);
    //         start_cycle_0 = snrt_mcycle();
    //         snrt_dma_start_1d_wideptr((uint64_t)(tcdm0_start_addr),
    //                                     tcdm1_start_addr,
    //                                     256 * sizeof(int8_t) * Test_Data_Rows);
    //         snrt_dma_wait_all();
    ...
    //      end_cycle_0 = snrt_mcycle();
    //      elapsed_0 = end_cycle_0 - start_cycle_0;

    //      // Print the result
    //      printf("[Time]: Copy + xDMA loopback took %u cycles\n", (unsigned int)elapsed_0);
    ...
    //     reshuffler_check_from_tcdm(
    //         (int8_t*)(tcdm0_start_addr+0x10000),
    //         C_golden,
    //         tbound_dst,
    //         tstride_dst,
    //         8,  // spatiial len
    ...
    //         uint32_t start_cycle = snrt_mcycle();
    //         snrt_dma_start_2d(
    //             (uint8_t *)(tcdm0_start_addr + 75536), // dst_offset
    //             (uint8_t *)(tcdm1_start_addr + 75536),  // src_offset
    //             sizeof(int8_t),
    //             1, //spatialStride_out,         
    //             1, //spatialStride_in,          
    //             64  // spatial length  8 
    ...
    //         uint32_t end_cycle = snrt_mcycle();
    //         uint32_t elapsed = end_cycle - start_cycle; 
    //         printf("[test1]: Baseline iDMA 2D transfer took %u cycles\n", (unsigned int)elapsed);
        ...
        if (snrt_is_dm_core()) {
            ...
            for (int i0 = 0; i0 < tbound_src[0]; ++i0) {
                ...
                uint32_t src_offset =
                    ...
                    // i4 * tstride_src[4] +
                    // i5 * tstride_src[5];
                // printf("idx[%d]: src_offset=%d dst_offset=%d\n", linear_idx, src_offset, dst_offset);
                
                snrt_dma_start_2d(
                    (uint8_t *)(tcdm0_start_addr + dst_offset), // dst_offset
                    (uint8_t *)(tcdm1_start_addr + src_offset),  // src_offset
                    8*sizeof(int8_t),
                    spatialStride_out, //spatialStride_out,         
                    spatialStride_in, //spatialStride_in,          
                    8  // spatial length  8 
                );
                snrt_dma_wait_all();
            }
