...
def emit_random_data_header(**kwargs):
    ...
    for j in range(B):
        for i in range(matrix_cols//32):
            tcdm_matrix[matrix_cols//32*j+i, 0:128] = matrix[8*j:8*j+8 , 32*i:32*i+16].reshape(-1)
            tcdm_matrix[matrix_cols//32*j+i, 128:256] = matrix[8*j:8*j+8 , 32*i+16:32*i+32].reshape(-1)
    ...
    header_lines += [
        ...
        # my_format_vector_definition("int8_t", "TCDM_Matrix", tcdm_matrix.reshape(-1), cols=16*8)
    ]
