...
def emit_random_data_header(**kwargs):
    ...
    tcdm_matrix = np.zeros((total_elems//(256), 256), dtype=np.int8)
    

    for j in range(matrix_rows//4):
        for i in range(matrix_cols//8//8):   # 32/8
            for k in range(8):
                tcdm_matrix[matrix_cols//8//8*j+i, 32*k:32*k+32] = matrix[4*j:4*j+4, 64*i+8*k:64*i+8*k+8].reshape(-1)
            
                # tcdm_matrix[4*j+i, 0:32   ] = matrix[4*j:4*j+4 , 64*i    :64*i+8  ].reshape(-1)
                # tcdm_matrix[4*j+i, 32:64  ] = matrix[4*j:4*j+4 , 64*i+8  :64*i+16 ].reshape(-1)
                # tcdm_matrix[4*j+i, 64:96  ] = matrix[4*j:4*j+4 , 64*i+16 :64*i+24 ].reshape(-1)
                # tcdm_matrix[4*j+i, 96:128 ] = matrix[4*j:4*j+4 , 64*i+24 :64*i+32 ].reshape(-1)
                # tcdm_matrix[4*j+i, 128:160] = matrix[4*j:4*j+4 , 64*i+32 :64*i+40 ].reshape(-1)
                # tcdm_matrix[4*j+i, 160:192] = matrix[4*j:4*j+4 , 64*i+40 :64*i+48 ].reshape(-1)
                # tcdm_matrix[4*j+i, 192:224] = matrix[4*j:4*j+4 , 64*i+48 :64*i+56 ].reshape(-1)
                # tcdm_matrix[4*j+i, 224:256] = matrix[4*j:4*j+4 , 64*i+56 :64*i+64 ].reshape(-1)
    ...
    if np.array_equal(c_golden,data):
        equal = 1
    else:
        equal = 0
    ...
    header_lines += [
        ...
        format_scalar_definition("int32_t", "equal", equal)
    ]
