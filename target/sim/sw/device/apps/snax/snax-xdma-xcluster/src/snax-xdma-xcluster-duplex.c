...
int main() {
    ...
    size_t dma_transfer_offset = 0x1000;
    ...
    if (snrt_cluster_idx() == 0) {
        if (snrt_is_dm_core()) {
            test_data_start_addr_0 = (uint64_t)Test_Data;
            ...
            snrt_dma_start_1d_wideptr((uint64_t)(tcdm0_start_addr+dma_transfer_offset), test_data_start_addr_0,
            Test_Data_Len);
            snrt_dma_wait_all();
        }
    }
    ...
    if (snrt_is_dm_core()) {
        if (snrt_cluster_idx() == 0) {
            printf("[C0] Start to load data from C1: %p -> %p\r\n",
                   (uint8_t*)tcdm1_start_addr, (uint8_t*)tcdm0_start_addr);
        ...
        if (snrt_cluster_idx() == 1) {
            printf("[C1] Start to load data from C0: %p -> %p\r\n",
                   (uint8_t*)tcdm0_start_addr, (uint8_t*)tcdm1_start_addr);
            ...
            // Setup transfer from C0 to C1
            xdma_memcpy_1d((uint8_t*)(tcdm0_start_addr+dma_transfer_offset), (uint8_t*)(tcdm1_start_addr+dma_transfer_offset), data_transfer_len * sizeof(int8_t));
    ...
    // Global barrier to ensure all transfers complete
    snrt_global_barrier();
    ...
    if (snrt_cluster_idx() == 0 && snrt_is_dm_core()) {
        ...
        for (int i = 0; i < data_transfer_len; i++) {
            if (((int8_t*)tcdm0_start_addr)[i] != ((int8_t*)tcdm1_start_addr)[i]) {
                err++;
                printf("C0 data is incorrect!\r\n");
                printf("tcdm0[%d]=%d, test_data_1[%d]=%d\r\n", i,
                       ((int8_t*)tcdm0_start_addr)[i], i,  ((int8_t*)tcdm1_start_addr)[i]);
                // return -1;
            }
        }
    ...
    //     for (int i = 0; i < data_transfer_len; i++) {
    //         if (((int8_t*)(tcdm1_start_addr+dma_transfer_offset))[i] != Test_Data[i]) {
    //             err++;
    //             printf("C1 data is incorrect!\r\n");
    //             // printf("tcdm1[%d]=%d, test_data_1[%d]=%d\r\n", i,
    //             //        ((int8_t*)(tcdm1_start_addr)[i+dma_transfer_offset], i, Test_Data[i]));
    //             // // return -1;
    //         }
    //     }
