...
def emit_random_data_header(**kwargs):
    ...
    systolic_dim = kwargs["systolic_dim"]  # 32

    block_elems = systolic_dim * matrix_rows
    ...
    matrices=[]
    for i in range(matrix_cols//systolic_dim):
        # Generate random data
        data = np.random.randint(MIN, MAX, block_elems, dtype=np.int8)
        matrix = data.reshape((matrix_rows, systolic_dim))

        systolic_rows = (systolic_dim // 8 - 1) + matrix_rows  # 8 bytes per bank
        systolic_cols = systolic_dim
        ...
        block_rows = tcdm_matrix.shape[0]
        matrices.append(tcdm_matrix)

    tcdm = np.vstack(matrices)
    ...
    header_lines += [
        format_scalar_definition("int32_t", "Block_Data_Rows", block_rows),
        format_scalar_definition("int32_t", "Test_Data_Rows", tcdm.shape[0]),
        my_format_vector_definition("int8_t", "Test_Data", tcdm.reshape(-1), cols=256)
    ]
