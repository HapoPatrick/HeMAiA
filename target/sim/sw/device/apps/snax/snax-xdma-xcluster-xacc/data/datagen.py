...
def emit_random_data_header(**kwargs):
    ...
    ndim_in = (kwargs["temp_dim_in"])
    ndim_out = (kwargs["temp_dim_out"])
    ...
    systolic_rows = (systolic_dim - 1) + matrix_rows
    systolic_cols = matrix_cols
    
    systolic_matrix = np.zeros((systolic_rows, systolic_cols), dtype=np.int8)

    for i in range(systolic_cols):
        systolic_matrix[i : matrix_rows + i, i] = matrix[:, i]
    ...
    header_lines += [
        ...
        format_scalar_definition("int32_t", "Test_Data_Rows", systolic_rows)
    ]

    padded_data = pad_elements_with_zeros(systolic_matrix.reshape(-1))
    padded_data = np.array(padded_data).reshape((systolic_rows, matrix_cols*8))
    # padded_data = duplicate_left_right(padded_data)  # Concatenate horizontally
    # padded_data = np.array(padded_data).reshape((systolic_rows, matrix_cols*8*2))


    if padded_data.shape[1] < 256:
        padding_cols = 256 - padded_data.shape[1]
        padded_data = np.pad(padded_data, ((0, 0), (0, padding_cols)), mode='constant', constant_values=0)
        

    header_lines += [
        my_format_vector_definition("int8_t", "Test_Data", padded_data.reshape(-1), cols=256)
    ]
